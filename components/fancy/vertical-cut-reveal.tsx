...
const VerticalCutReveal = forwardRef<VerticalCutRevealRef, TextProps>(
  (
    ...
    const variants = {
      hidden: { y: reverse ? "-101%" : "101%" },
      visible: (i: number) => ({
        y: 0,
        transition: {
          ...transition,
          delay: ((transition?.delay as number) || 0) + staggerDelay + getStaggerDelay(i),
        },
      }),
    }
        ...
        {(splitBy === "characters"
          ...
          return (
            <span
              key={wordIndex}
              aria-hidden="true"
              className={cn("inline-flex overflow-hidden pb-[0.1em] pr-[0.01em]", wordLevelClassName)}
            >
